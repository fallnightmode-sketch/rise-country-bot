...
# ====================================================================
# AUTOMATED SCHEDULER EXECUTIONS (CRON-BASED JOBS)
# ====================================================================
...
class SessionPlannerPage2Modal(discord.ui.Modal, title="Page 2: Milestone Configurations"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # PARSING ABSOLUT JAM STAFF JOIN MENJADI INT (UNTUK CRON TRIGGER)
        try:
            s_time_clean = re.sub(r'[^0-9.]', '', self.f_staff.value.strip().replace(":", "."))
            s_hour, s_min = map(int, s_time_clean.split('.'))
        except Exception: 
        ...
        # PARSING ABSOLUT JAM OPEN SERVER MENJADI INT (UNTUK CRON TRIGGER)
        try:
            o_time_clean = re.sub(r'[^0-9.]', '', self.f_open.value.strip().replace(":", "."))
            o_hour, o_min = map(int, o_time_clean.split('.'))
        except Exception: 
