...
class RejectReasonModal(discord.ui.Modal, title="LOA Rejection Reason"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if member:
            try:
                embed_dm = discord.Embed(title="Your LOA Request Has Been Rejected", description=f"Reason: {self.reason.value}", color=discord.Color.red())
                await member.send(embed=embed_dm)
            except Exception: pass
...
class AdminApprovalView(discord.ui.View):
    ...
    async def approve_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if member:
            ...
            try:
                embed_dm = discord.Embed(title="Your LOA Request Has Been Approved", description=f"End Date: {self.data_form['end_date']}", color=discord.Color.green())
                await member.send(embed=embed_dm)
            except Exception: pass
...
class LOAForm(discord.ui.Modal, title="Leave of Absence Application"):
    ...
    q3 = discord.ui.TextInput(label="3. LOA End Date (DD/MM/YYYY)", required=True, max_length=15)
    q4 = discord.ui.TextInput(label="4. Reason & Notes", style=discord.TextStyle.long, required=True, max_length=400)
    q5 = discord.ui.TextInput(label="5. Reachable during leave? (Yes / No)", required=True, max_length=300)
    async def on_submit(self, interaction: discord.Interaction):
        if interaction.guild_id != GUILD_ID: return
...
# ====================================================================
# FUNGSI CRON EKSEKUSI OTOMATIS (BENTUKAN LAMA)
# ====================================================================
...
# ====================================================================
# TWO-STAGE MODAL (KEMBALI KE STRUKTUR LAMA: PARSING STRING MANUAL)
# ====================================================================
class SessionPlannerPage2Modal(discord.ui.Modal, title="Page 2: Milestone Configurations"):
    f_staff = discord.ui.TextInput(label="1) Staff Join Time (HH.MM)", placeholder="Contoh: 21.00", required=True)
    f_open = discord.ui.TextInput(label="2) Open Server Time (HH.MM)", placeholder="Contoh: 21.05", required=True)
    f_sts = discord.ui.TextInput(label="3) STS Time (HH.MM)", placeholder="Contoh: 21.10", required=True)
    f_rp_start = discord.ui.TextInput(label="4) Roleplay Start Time (HH.MM)", placeholder="Contoh: 21.15", required=True)
    f_end = discord.ui.TextInput(label="5) End Session Time (HH.MM)", placeholder="Contoh: 22.00", required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # BENTUKAN LAMA: Pemisahan string menggunakan regex / split titik biasa
        try:
        ...
        if announcement_channel:
            ...
            # KEMBALI KE TEMPLATE SCHEDULER 'cron' ASLI YANG BISA JALAN DULU
...
class SessionPlannerPage1Modal(discord.ui.Modal, title="Page 1: Identity & Parameters"):
    f_host = discord.ui.TextInput(label="Host Name", required=True, max_length=100)
    f_map = discord.ui.TextInput(label="Map Author Credit", required=True, max_length=100)
    f_day_date = discord.ui.TextInput(label="Day & Date", required=True, max_length=100)
    f_aorp = discord.ui.TextInput(label="AORP Location / City", required=True, max_length=100)
    f_code = discord.ui.TextInput(label="Server Private Code / Link", required=True, max_length=200)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        class NextStageView(discord.ui.View):
            ...
            @discord.ui.button(label="Proceed to Milestone Configuration", style=discord.ButtonStyle.primary)
        ...
        await interaction.response.send_message(content="Data awal dicatat. Klik tombol di bawah:", view=NextStageView(), ephemeral=True)
...
async def start_session_planner(ctx):
    ...
    await ctx.send(content="Pilih channel tujuan format:", view=WizardTriggerView())

@bot.command(name="end_loa")
@commands.has_permissions(administrator=True)
async def remove_loa_manual(ctx, member: discord.Member = None):
    if ctx.guild is None or ctx.guild.id != GUILD_ID: return
    if member is None: return await ctx.send("Error. Masukkan user.")
    guild = ctx.guild
    role_loa = guild.get_role(ID_ROLE_LOA)
    ...
    if str(member.id) in loa_data:
        ...
        save_loa_data(loa_data)
    await ctx.send(f"LOA selesai manual untuk {member.display_name}")
...
async def toggle_loa_system(ctx, status: str = None):
    ...
    await ctx.send(f"Sistem LOA diubah ke {loa_system_active}")
...
async def on_ready():
    ...
    print(f"Bot siap menggunakan struktur lama.")
